...
class RecicladorService:
    ...
    def login(self, nit: int):
        ...
        reciclador = self.session.exec(
            select(RecicladoresModel)
            .where(
                RecicladoresModel.nit == nit
            )
        ).first()
    ...
    def agregar_puntos(
        ...
        puntos = (
            Decimal(str(material.gramos))
            * categoria.puntos_por_gramo
        )
        ...
        movimiento = MovimientoPuntos(
            resiclador_nit=nit,
            puntos=puntos,
            motivo=f"Reciclaje {categoria.nombre}"
        )
